...
def print():
    ...
    heading.pack()
    heading.config(bg="light yellow")
    ...
    for child in listBox.get_children():
        ...
        item1.config(bg="light yellow")
        item1.pack()
 
    tot = Label(top, text=f'Total\t{tott}')
    tot.config(bg="orange")
    tot.pack()
...
def show():
    ...
    if (var1.get()):
        ...
        qty = int(e6.get())
        tot = int(price * qty)
        tempList = [['Product 1', e1.get(), e6.get(), tot]]
    ...
    if (var2.get()):
        ...
        qty = int(e7.get())
        tot = int(price * qty)
        tempList = [['Product 2 ', e2.get(), e7.get(), tot]]
        tempList.sort(key=lambda e: e[1], reverse=True)
        for i, (item, price, qty, tot) in enumerate(tempList, start=1):
            listBox.insert("", "end", values=(item, price, qty, tot))
    ...
    if (var3.get()):
        ...
        qty = int(e8.get())
        tot = int(price * qty)
        tempList = [['Product 3 ', e3.get(), e8.get(), tot]]
        tempList.sort(key=lambda e: e[1], reverse=True)
        for i, (item, price, qty, tot) in enumerate(tempList, start=1):
            listBox.insert("", "end", values=(item, price, qty, tot))
    ...
    if (var4.get()):
        ...
        qty = int(e9.get())
        tot = int(price * qty)
        tempList = [['Product 4', e4.get(), e9.get(), tot]]
        tempList.sort(key=lambda e: e[1], reverse=True)
    ...
    if (var5.get()):
        ...
        qty = int(e10.get())
        tot = int(price * qty)
        tempList = [['Product 5', e5.get(), e10.get(), tot]]
        tempList.sort(key=lambda e: e[1], reverse=True)
        for i, (item, price, qty, tot) in enumerate(tempList, start=1):
            listBox.insert("", "end", values=(item, price, qty, tot))
...
root = Tk()
root.title("Inventory Management System using Python")
...
Label(root, text="Inventory Management System using Python", font="arial 20 bold" ,bg="Yellow").place(x=5, y=10)
 
 
var1 = IntVar()
Checkbutton(root, text="Product 1", variable=var1).place(x=10, y=50)
 
var2 = IntVar()
Checkbutton(root, text="Product 2", variable=var2).place(x=10, y=80)
 
var3 = IntVar()
Checkbutton(root, text="Product 3", variable=var3).place(x=10, y=110)
 
var4 = IntVar()
Checkbutton(root, text="Product 4", variable=var4).place(x=10, y=140)
 
var5 = IntVar()
Checkbutton(root, text="Product 5 ", variable=var5).place(x=10, y=170)
Label(root, text="Total").place(x=660, y=10)
